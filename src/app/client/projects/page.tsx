...
import { ProjectContributorsButton } from "./project-contributors-button";
...
export default async function ClientProjectsPage({
  ...
  const projectIds = clientAssignments.map((a) => a.projectId);
  ...
  const [projectsTotal, projects, contributorAssignments] = await Promise.all([
  ...
  const contributorsByProject = new Map<string, Array<{ id: string; displayName: string; email: string; username: string }>>();
  for (const a of contributorAssignments) {
    const list = contributorsByProject.get(a.projectId) ?? [];
    list.push(a.contributor);
    contributorsByProject.set(a.projectId, list);
  }
        ...
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              ...
              {projects.map((p) => {
                const assigned = contributorsByProject.get(p.id) ?? [];
                return (
                  <TableRow key={p.id}>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <Link href={`/client/projects/${p.id}`} className="font-medium text-slate-900 hover:text-indigo-600 transition-colors">
                          {p.name}
                        </Link>
                      </div>
                    </TableCell>
                    ...
                    <TableCell className="sticky right-0 bg-white">
                      <div className="flex items-center gap-2">
                        ...
                        <ProjectContributorsButton
                          projectId={p.id}
                          assignedContributors={assigned}
                        />
                      </div>
                    </TableCell>
                  </TableRow>
