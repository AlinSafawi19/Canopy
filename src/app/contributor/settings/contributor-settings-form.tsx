...
interface Contributor { id: string; displayName: string; email: string; username: string }

export function ContributorSettingsForm({ contributor }: { contributor: Contributor }) {
  const router = useRouter();
  const [form, setForm] = useState({ displayName: contributor.displayName, email: contributor.email });
  ...
  async function saveProfile(e: React.FormEvent<HTMLFormElement>) {
    ...
    setLoading(true);
    const res = await apiFetch("/api/contributor/profile", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
    setLoading(false);
    setMsgOk(res.ok);
  ...
  async function changePassword(e: React.FormEvent<HTMLFormElement>) {
    ...
    setPwLoading(true);
    setPwMsg("");
    const res = await apiFetch("/api/contributor/profile", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ currentPassword: pw.current, newPassword: pw.next }) });
    setPwLoading(false);
    setPwMsgOk(res.ok);
