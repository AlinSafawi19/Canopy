...
export function CreateContributorButton() {
  ...
  return (
    <>
      ...
      <Modal ref={modalRef} open={open} onClose={() => { setOpen(false); reset(); }} title="Create Contributor" isDirty={touched} busy={loading}
        footer={
          <div className="flex justify-end gap-3">
            <Button variant="outline" type="button" onClick={() => modalRef.current?.attemptClose()}>Cancel</Button>
            <Button type="submit" form="create-contributor-form" loading={loading}>Create contributor</Button>
          </div>
        }
      >
        <form id="create-contributor-form" onSubmit={handleSubmit} className="space-y-4" onInput={() => setTouched(true)}>
