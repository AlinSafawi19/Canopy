...
interface Admin { id: string; displayName: string; email: string; username: string; tenantId: string }

export function AdminSettingsForm({ admin }: { admin: Admin }) {
  const router = useRouter();
  const [form, setForm] = useState({ displayName: admin.displayName, email: admin.email });
    ...
    <div className="space-y-8">
      <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg">
        <div>
          ...
          <Badge variant="info" className="mt-1">{admin.tenantId}</Badge>
        </div>
      </div>
