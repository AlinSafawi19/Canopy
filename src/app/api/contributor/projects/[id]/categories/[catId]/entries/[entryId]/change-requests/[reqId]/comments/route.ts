...
async function getAuthorizedCR(reqId: string, entryId: string, catId: string, projectId: string, contributorId: string) {
  const assignment = await prisma.contributorAssignment.findFirst({ where: { contributorId, projectId } });
...
export async function GET(request: NextRequest, { params }: Ctx) {
  ...
  const cr = await getAuthorizedCR(reqId, entryId, catId, projectId, session.id);
...
export async function POST(request: NextRequest, { params }: Ctx) {
  ...
  try {
    ...
    console.error("[contributor/change-request-comments POST]", err);
