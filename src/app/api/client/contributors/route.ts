...
export async function POST(request: NextRequest) {
  ...
  try {
    const { username, password, displayName, email, projectId } = await request.json();
    ...
    if (existing) {
      return NextResponse.json({ error: "Username is already taken in this workspace." }, { status: 409 });
    }
    ...
    await prisma.contributorAssignment.create({
      data: { contributorId: contributor.id, projectId },
    });
    ...
    return NextResponse.json({ id: contributor.id }, { status: 201 });
  } catch (err) {
