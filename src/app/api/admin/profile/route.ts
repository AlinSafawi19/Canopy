...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    if (body.newPassword) {
      ...
      await prisma.adminIdentity.update({ where: { id }, data: { password: await hashPassword(body.newPassword) } });
      sendSecurityAlertEmail(admin.email, admin.displayName, "password_changed").catch(() => {});
    } else {
